...
floor = pygame.image.load("assets/Floor Grass - Copy (2).png")
Level = 1
...
while True:
    ...
    screen.blit(floor, [floor_x, 298])
    screen.blit(floor, [floor_2x, 298])
    screen.blit(floor, [floor_3x, 298])
    ...
    if floor_x <= -1 * floor_width:
        floor_x = 3 * floor_width + floor_x
    if floor_2x <= -1 * floor_width:
        floor_2x = 3 * floor_width + floor_2x
    if floor_3x <= -1 * floor_width:
        floor_3x = 3 * floor_width + floor_3x
    floor_3x = floor_3x - 4
    ...
    if Menu == "Main":
        screen.blit(Menu_Assets[1], (700, 100, 0, 0))
